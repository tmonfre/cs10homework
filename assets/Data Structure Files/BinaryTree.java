...
public class BinaryTree<E> {
	...
	/**
	 * Produces a "shallow" copy of a tree, down to the given depth
	 */
	public BinaryTree<E> copyToDepth(int d) {
		// return a single new tree (with height 0) if d==0
		if (d == 0) return new BinaryTree<E>(data);
		
		// initialize l, and recursively call copyToDepth, decrementing d if a left child exists
		BinaryTree<E> l = null;
		if (hasLeft()) l = left.copyToDepth(d-1); // call copy to depth on next level of tree
		
		// initialize r, and recursively call copyToDepth, decrementing d if a right child exists
		BinaryTree<E> r = null;
		if (hasRight()) r = right.copyToDepth(d-1); // call copy to depth on next level of tree
		
		// return copy of the shallow tree that is recursively cut off
