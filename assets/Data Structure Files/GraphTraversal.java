...
public class GraphTraversal<V,E> {
	...
	public ArrayList<V> findPath(V start, V end) {
		//check that DFS or BFS have already been run
		if (backTrack.isEmpty()) {
			System.out.println("Run DFS or BFS before trying to find a path");
			return new ArrayList<V>();
		}
		System.out.println("Path from " + start + " to " + end);
		//make sure both the start and end vertices are in backTrack
		if (!backTrack.containsKey(start) || !backTrack.containsKey(end)) {
	...
	public static void main(String[] args) {
		...
		//run DFS from A and find path to H
		GT.DFS(g2,"A");
		GT.findPath("A", "H");
		
		//run BFS from A and find path to H
		GT.BFS(g2,"A");
		GT.findPath("A", "H");
